...
%if enable_streaming_interface == str(1):
  ...
  %for c in range(n_pea_cols):
    reg_sel_out_col_pea_o[${c}] = reg2hw.sel_out_col_pea[0].sel_col_${c};
  %endfor
  %for r in range(n_pea_rows):
    %for c in range(n_pea_cols):
    reg_acc_value_pe_o[${r}][${c}] = reg2hw.acc_value[${r}].pe_${c};
    %endfor 
  %endfor
%endif
    ////////////////////////////////////////////////////////////////
...
%for r in range(n_pea_rows):
  %for c in range(n_pea_cols):
    reg_pea_constants_o[${r}][${c}] = reg2hw.pea_constants[${r}].c${c}.q; 
  %endfor
%endfor
